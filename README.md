...
### 2016.06.07
- 添加证用病机、多级复合病机以及复合病机近似度
- 更新外感病名选择证名存储过程(p_wg_bm_zm_tl)，若病名、证名成立且对应则将病历状态置为0；若病名、证名成立但不对应则将病历状态置为1.
 - 说明：该过程中的病名和证名包括近似推理而得的。
- 更新内伤病名选择证名存储过程(p_ns_bm_zm_tl)，若病名、证名成立且对应则将病历状态置为0；若病名、证名成立但不对应则将病历状态置为1.
 - 说明：该过程中的病名和证名包括近似推理而得的。
